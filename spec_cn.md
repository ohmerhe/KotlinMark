...
基于在电子邮件中指示格式化的约定
和usenet帖子。 它是由约翰·格鲁伯（John Gruber）开发的(在Aaron Swartz的帮助下)，
在2004年通过「[语法描述](http://daringfireball.net/projects/markdown/syntax)」
和「一个将Markdown转换为 HTML 的 Perl 脚本（`Markdown.pl`）」的形式发布。
在接下来的十年中，有几十个语言的实现版本。
有的扩展了原来的 Markdown 语法，包含对脚注，表和其他文档元素的约定。
有些允许Markdown文档以 HTML 以外的格式呈现。
如网站Reddit、StackOverflow 和 GitHub 有数百万人使用 Markdown。
而 Markdown 开始在互联网之外被使用，作者的书籍、文章、幻灯片、信件和讲义。

将 Markdown 与许多其他轻量级标记语法（通常更容易编写）区分开来是其可读性。
正如Gruber写道：

> Markdown格式化语法的首要设计目标是使其尽可能可读。这个想法是，Markdown格式的文档应该是纯文本原样发布的，而不是像标签或格式化说明一样被标记。(<http://daringfireball.net/projects/markdown/>)
